...
module.exports = (() => {
  var _ref = _asyncToGenerator(function* (collection, params) {
    ...
    let results;
    if (findMethod === 'findAsCursor') {
      results = yield collection['findAsCursor']({ $and: [cursorQuery, params.query] }, params.fields).sort($sort).limit(params.limit + 1) // Query one more element to see if there's another page.
      .toArray();
    } else {
      results = yield collection['find']({ $and: [cursorQuery, params.query] }, {
